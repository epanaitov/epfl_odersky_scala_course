...
class Empty extends TweetSet {
  ...
  override def union(that: TweetSet): TweetSet = if (that.isEmpty) new Empty else that
  
  override def ascendingByRetweet: Trending = new EmptyTrending
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    ...
    def filter1(p: Tweet => Boolean, accu: TweetSet, set1: TweetSet): TweetSet = {
      if (set1.isEmpty) accu
      else if (p(set1.head)) filter1(p, accu.incl(set1.head), set1.tail)
      else filter1(p, accu, set1.tail)
    }
  ...
  override def union(that: TweetSet): TweetSet = {
      ...
      def union0(set1: TweetSet, accu: TweetSet): TweetSet = {
        if (set1.isEmpty) accu
        else union0(set1.tail, accu.incl(set1.head))
      }
      
      if (that.isEmpty) this
      else union0(this, that)
    }
  
  override def ascendingByRetweet: Trending = {
    
    def acc0(set1: TweetSet, accu: Trending): Trending = {
      if (set1.isEmpty) accu
      else {
        val minRetweet = set1.findMin
        acc0(set1.remove(minRetweet), accu + minRetweet)
      }
    }
    
    acc0(this, new EmptyTrending)
  }
}
...
object GoogleVsApple {
  ...
  def testList(text: String, list: List[String]): Boolean = {
    if (list.isEmpty) false
    else if (text.contains(list.head)) true
    else testList(text, list.tail)
  }

  val googleTweets: TweetSet = TweetReader.allTweets.filter(x => testList(x.text, google))
  val appleTweets: TweetSet = TweetReader.allTweets.filter(x => testList(x.text, apple))
...
object Main extends App {
  ...
  val set1 = new NonEmpty(new Tweet("user", "tweet", 0), new Empty, new Empty)
  val set2 = new NonEmpty(new Tweet("user", "atweet", 1), new Empty, new Empty)
  val united = set1 union set2
  
  united.ascendingByRetweet.foreach(println)
}
